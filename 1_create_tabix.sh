#!/bin/bash
#SBATCH -p c2s4
#SBATCH --job-name=EA_O_tab
#SBATCH --out=EA_O_tab.log
#SBATCH --error=EA_O_tab.err
...
base_dir=gcs_mnt/hdcekabarnesadrn1/ADRN_MEGA_META_SEVERITY/b_EA_OMNI/imputed
for i in {22..1}; do
  if [[ -f "${base_dir}/chr${i}.dose.vcf.gz.tbi" ]]; then continue; fi
  echo tabix -f "${base_dir}/chr${i}.dose.vcf.gz"
  tabix -f "${base_dir}/chr${i}.dose.vcf.gz"
done

# log files moved to ~/gcs_mnt/hdcekabarnesadrn1/ADRN_MEGA_META_SEVERITY/EOS_PRS/log
