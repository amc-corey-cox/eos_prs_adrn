...
```{r}
prs_violin_input <- function(.data, pheno, score, sev, scale = TRUE, perturb = FALSE, threshold = FALSE) {
  perturb <- runif(n = nrow(.data), min = -0.001, max = 0.001)
  .data %>%
    select(PHENO = !! ensym(pheno), SCORE = !! ensym(score), !! ensym(sev)) %>%
    mutate(Phen_num12 = PHENO - 1,
           PHENO = recode(PHENO, `1` = "Control", `2` = "Case"),
           SCORE = ifelse(perturb == TRUE, SCORE + perturb, SCORE),
           SCORE = ifelse(scale == TRUE, scale(SCORE), SCORE)) %>%
    filter(threshold == FALSE | !! ensym(sev) >= threshold | is.na(!! ensym(sev)))
...
eos_phenos_table <- eos_phenos %>%
  select(IID = SampleID, PHENO = Pheno_ADNA, SCORE, Age = AGE)

eos_phenos_table <- eos_phenos %>% prs_violin_input(Pheno_ADNA, SCORE, AGE)

gg_prs_violin_ca2(eos_phenos_table, sev = "AGE", plot_title = "MEGA EA EOS PRS vs AD Case/Control")
```



```{r new_fx}
gg_prs_violin_ca2 <- function(.data, sev, plot_title) {
sevTreshold = FALSE
